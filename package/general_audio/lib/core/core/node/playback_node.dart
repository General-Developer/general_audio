...
class PlaybackNode extends AudioNode
  ...
  @override
  late final inputBus =
      AudioInputBus(node: this, formatResolver: (_) => device.format);

  @override
  late final AudioEndpointBus outputBus =
      AudioEndpointBus(node: this, formatResolver: (_) => device.format);

  @override
  AudioReadResult read(AudioOutputBus outputBus, AudioBuffer buffer) {
    final writableFrameCount =
        min(buffer.sizeInFrames, device.availableWriteFrames);
    final readResult =
        inputBus.connectedBus!.read(buffer.limit(writableFrameCount));
